...
import { User, Mail, MessageSquare, Send, CheckCircle2, Loader2 } from 'lucide-react';
...
export function ContactForm(): React.ReactElement {
  ...
  return (
    <>
      <div className="flex flex-col justify-center p-8 md:p-10 bg-gray-50 dark:bg-zinc-800/50 rounded-r-2xl shadow-xs border-2 border-yellow-300 dark:border-yellow-700/60">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="mt-4 text-center">
            <p className="text-xs text-muted-foreground">Your information is securely processed and never shared with third parties.</p>
          </div>
        </form>
      </div>
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent className="max-w-md bg-card border-border">
          ...
          <AlertDialogFooter className="flex justify-center">
            <AlertDialogAction className="px-8 py-2.5 rounded-lg" onClick={() => setIsAlertOpen(false)}>
              Close
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      {/* Error Alert */}
      {status === 'error' && (
        <Alert variant="destructive" className="fixed bottom-4 right-4 max-w-md animate-in fade-in slide-in-from-bottom-5">
