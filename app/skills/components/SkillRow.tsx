...
export function SkillRow({ skill, index }: SkillRowProps) {
  ...
  const hasDarkIcon = ["nextjs", "vercel", "github", "express"].includes(
    skill.id,
  );
          ...
          >
            ...
            >
              <div className="w-full h-full flex items-center justify-center relative z-10">
                <Icon
                  className={`h-20 w-20 ${getIconColorClass()} drop-shadow-sm transition-transform duration-300 group-hover:scale-110`}
                />
              </div>
        ...
        <div className="flex-1 p-4 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            <div className="flex items-start justify-between">
              ...
              <div>
                {skill.learningStatus === "learning" ? (
                  ...
                  <span
                    className={`flex items-center gap-1 text-sm font-medium px-2.5 py-1 rounded-md ${getTimeBadgeColors()} shadow-sm`}
                  >
            ...
            <div className="flex flex-wrap gap-1">
              ...
              <Badge
                key={skill.category}
                variant="outline"
                className={`text-xs font-medium ${categoryBadgeColors[skill.category]}`}
              >
                {skill.category}
              </Badge>
              ...
              {skill.relatedCategories?.map((relatedCategory) => (
                <Badge
                  key={relatedCategory}
                  variant="outline"
                  className={`text-xs opacity-75 ${categoryBadgeColors[relatedCategory]}`}
                >
                  {relatedCategory}
                </Badge>
              ))}
          ...
          <div className="mt-4 flex justify-end">
            <Button
              ...
              variant="outline"
              asChild
              className={`border-2 ${getButtonColors()} font-medium text-sm shadow-sm transition-colors`}
            >
              <Link
                href={skill.officialUrl}
                target="_blank"
