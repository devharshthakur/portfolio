...
export function SkillCard({ skill, index }: SkillCardProps) {
  ...
  const isDarkIcon = ["nextjs", "vercel", "github", "express"].includes(
    skill.id,
  );
  ...
  return (
    <Card
      className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg m-1 border-4 border-border"
      ...
      <CardHeader className="pt-4 pb-0">
        ...
        <div className="flex flex-row justify-between">
          <div className="flex items-center space-x-3">
            ...
            >
              <Icon
                className={`h-6 w-6 ${skill.color.light} ${getEnhancedIconColor()}`}
              />
            </div>
            <div>
